...
async def agendar_consulta(
    ...
    try:
        ...
        medico = (
            db.query(models.User)
            .filter(
                models.User.id == agendamento_data.medico, models.User.role == "medico"
            )
            .first()
        )

        if not medico:
            return AgendamentoResponse(success=False, message="Médico não encontrado.")

        # Buscar sala
        sala = (
            db.query(models.Salas)
            .filter(models.Salas.id == agendamento_data.sala)
            .first()
        )
        if not sala:
            return AgendamentoResponse(success=False, message="Sala não encontrada.")

        # Buscar turno
        turno = (
            db.query(models.Turnos)
            .filter(models.Turnos.id == agendamento_data.turno)
            .first()
        )
        if not turno:
            return AgendamentoResponse(success=False, message="Turno não encontrado.")
        ...
        return AgendamentoResponse(
            success=False, message=f"Erro ao agendar consulta: {e!s}"
        )
...
async def get_dados_agendamento(
    ...
    try:
        ...
        vagas = (
            ...
            .filter(models.Vagas.clinica == 1)
            .all()
        )  # Clínica padrão

        # Buscar pacientes
        pacientes = (
            db.query(models.User).filter(models.User.role == "paciente").limit(50).all()
        )
...
async def garantir_turnos_padrao(db: Session):
    ...
    for turno_data in turnos_padrao:
        ...
        turno_existente = (
            db.query(models.Turnos)
            .filter(models.Turnos.nome == turno_data["nome"])
            .first()
        )
        ...
        if not turno_existente:
            ...
            if (
                ...
                or turno_existente.hora_fim != turno_data["hora_fim"]
            ):
                turno_existente.hora_inicio = turno_data["hora_inicio"]
