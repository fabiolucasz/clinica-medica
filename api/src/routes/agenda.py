...
async def get_agenda_completa(
    ...
    try:
        ...
        agendamentos_query = db.query(models.Agendamentos)\
            .options(
        ...
        if medico_id:
            agendamentos_query = agendamentos_query.filter(models.Agendamentos.medico == medico_id)
        ...
        for vaga in vagas:
            ...
            vaga_dict = {
                ...
                'turno_hora_inicio': vaga.turno_rel.hora_inicio if vaga.turno_rel else None,
        ...
        for agendamento in agendamentos:
            agendamento_dict = {
                ...
                'paciente_nome': agendamento.paciente_rel.nome if agendamento.paciente_rel else None,
                'medico_id': agendamento.medico,
                'medico_nome': agendamento.medico_rel.nome if agendamento.medico_rel else None,
                'sala_id': agendamento.sala,
                'sala_nome': agendamento.sala_rel.nome if agendamento.sala_rel else None,
                'turno_id': agendamento.turno,
                'turno_nome': agendamento.turno_rel.nome if agendamento.turno_rel else None,
        ...
        print(f"DEBUG: {len(medicos_data)} médicos, {len(vagas_data)} vagas, {len(agendamentos_data)} agendamentos")
