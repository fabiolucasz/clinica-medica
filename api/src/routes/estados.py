...
async def create_estado(estado: EstadoCreate, current_user: CurrentUser, db: SessionDep):
    return crud.create_estado(db, estado)

@router.put("/estados/{id}", response_model=Estados)
async def update_estado(id: int, estado: EstadoUpdate, current_user: CurrentUser, db: SessionDep):
