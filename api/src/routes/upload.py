...
async def upload_profile_image(
    ...
    if file_ext not in allowed_extensions:
        raise HTTPException(
            status_code=400, detail="Formato não permitido. Use: PNG, JPEG, JPG"
        )
    ...
    success, message, url = upload_file(
        file_content=content, user_id=user_id, file_type="image", folder="profiles"
    )
...
async def upload_professional_document(
    ...
    success, message, url = upload_file(
        file_content=content, user_id=user_id, file_type="document", folder=folder
    )
...
async def delete_uploaded_file(
    file_url: str, current_user: models.User = Depends(get_current_user)
):
    """
