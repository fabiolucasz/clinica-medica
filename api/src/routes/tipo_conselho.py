...
async def create_tipo_conselho(tipo_conselho: TiposConselhoCreate, current_user: CurrentUser, db: SessionDep):
    return crud.create_tipo_conselho(db, tipo_conselho)

@router.put("/tipo-conselho/{id}")
async def update_tipo_conselho(id: int, tipo_conselho: TiposConselhoUpdate, current_user: CurrentUser, db: SessionDep):
