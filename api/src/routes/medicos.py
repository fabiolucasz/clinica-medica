...
async def get_medicos(request: Request, current_user: CurrentUser, db: SessionDep):
    ...
    try:
        ...
        log_user_operation(
            operation="get_medicos", user_id=current_user.id, success=True
        )
...
async def create_medico(
    medico: MedicoCreate, current_user: CurrentUser, db: SessionDep
):
    # Verificar se o usuário é admin
    if current_user.role != "administrador":
        raise HTTPException(
            status_code=403,
            detail="Acesso negado. Apenas administradores podem cadastrar médicos.",
        )
...
async def update_medico(
    id: int, medico: MedicoUpdate, current_user: CurrentUser, db: SessionDep
):
    # Verificar se o usuário é admin
    if current_user.role != "administrador":
        raise HTTPException(
            status_code=403,
            detail="Acesso negado. Apenas administradores podem editar médicos.",
        )
...
async def delete_medico(id: int, current_user: CurrentUser, db: SessionDep):
    ...
    if current_user.role != "administrador":
        raise HTTPException(
            status_code=403,
            detail="Acesso negado. Apenas administradores podem excluir médicos.",
        )
