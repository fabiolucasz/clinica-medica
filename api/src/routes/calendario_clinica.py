...
async def get_calendario_clinica_by_clinica(
...
async def get_calendario_clinica_by_data(
    data: str, current_user: CurrentUser, db: SessionDep
):
    return crud.get_calendario_clinica_by_data(db, data)


@router.post("/calendario-clinica/")
async def create_calendario_clinica(
    calendario_clinica: CalendarioClinicaCreate,
...
async def update_calendario_clinica(
    id: int,
    calendario_clinica: CalendarioClinicaUpdate,
    current_user: CurrentUser,
    db: SessionDep,
):
