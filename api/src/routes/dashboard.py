...
async def get_dashboard_resumo(current_user: CurrentUser, db: SessionDep):
    ...
    try:
        ...
        total_pacientes = (
            db.query(models.User).filter(models.User.role == "paciente").count()
        )
        total_medicos = (
            db.query(models.User).filter(models.User.role == "medico").count()
        )
        ...
        consultas_pendentes = (
            db.query(models.Agendamentos)
            .filter(models.Agendamentos.status == "aguardando")
            .count()
        )
        ...
        consultas_mes = (
            db.query(models.Agendamentos)
            .filter(models.Agendamentos.data_consulta >= inicio_mes)
            .count()
        )
        ...
        ultimas_consultas = (
            ...
            .order_by(models.Agendamentos.data_consulta)
            .limit(5)
            .all()
        )
        ...
        for consulta in ultimas_consultas:
            paciente_nome = (
                consulta.paciente_rel.nome if consulta.paciente_rel else "N/A"
            )
            ...
            pacientes_formatados.append(
                {
                    ...
                    "data_cadastro": (
                        paciente.created_at.isoformat() if paciente.created_at else None
                    ),
                }
            )
        ...
        status_counts = (
            db.query(models.Agendamentos.status, func.count(models.Agendamentos.id))
            .group_by(models.Agendamentos.status)
            .all()
        )
        ...
        for i in range(5, -1, -1):
            ...
            grafico_mensal.append(
                {"mes": mes_data.strftime("%b/%Y"), "quantidade": count}
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao gerar resumo do dashboard: {e!s}"
        )
