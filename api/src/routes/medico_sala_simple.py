...
async def get_medico_sala_simple(
    medico_id: int, current_user: CurrentUser, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        for vaga in vagas:
            vaga_dict = {
                ...
                "sala": vaga.sala,
                "sala_nome": (
                    vaga.sala_rel.nome if vaga.sala_rel else "Sala não encontrada"
                ),
                "turno": vaga.turno,
        ...
        print("DEBUG SIMPLES: Retornando resultado")
        return resultado

    except Exception as e:
        print(f"DEBUG SIMPLES: Erro: {e!s}")
        import traceback
