...
async def create_especialidade(especialidade: EspecialidadesCreate, current_user: CurrentUser, db: SessionDep):
    return crud.create_especialidade(db, especialidade)

@router.put("/especialidades/{id}")
async def update_especialidade(id: int, especialidade: EspecialidadesUpdate, current_user: CurrentUser, db: SessionDep):
