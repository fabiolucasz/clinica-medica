...
async def get_agendamentos(db: SessionDep):
    return crud.get_agendamentos(db)

@router.get("/agendamentos/{id}")
async def get_agendamento(id: int, db: SessionDep):
    return crud.get_agendamento(db, id)

@router.post("/agendamentos/")
async def create_agendamento(agendamento: AgendamentosCreate, db: SessionDep):
    return crud.create_agendamento(db, agendamento)

@router.put("/agendamentos/{id}")
async def update_agendamento(id: int, agendamento: AgendamentosUpdate, db: SessionDep):
    return crud.update_agendamento(db, id, agendamento)

@router.delete("/agendamentos/{id}")
async def delete_agendamento(id: int, db: SessionDep):
