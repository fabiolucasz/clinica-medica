...
def get_especialidade(db: SessionDep, id: int):
    return (
        db.query(models.Especialidades).filter(models.Especialidades.id == id).first()
    )
...
def update_especialidade(db: SessionDep, id: int, especialidade: EspecialidadesUpdate):
    db.query(models.Especialidades).filter(models.Especialidades.id == id).update(
        especialidade.model_dump()
    )
    db.commit()
    return (
        db.query(models.Especialidades).filter(models.Especialidades.id == id).first()
