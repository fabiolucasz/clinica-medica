...
from src.schemas.calendario_clinica import (
    CalendarioClinicaCreate,
    CalendarioClinicaUpdate,
)


def get_calendario_clinica(db: Session):
    results = (
        db.query(models.CalendarioClinica, models.Calendario.data_br)
        .join(
            models.Calendario,
            models.CalendarioClinica.data_feriado == models.Calendario.id,
        )
        .all()
    )
...
def update_calendario_clinica(
    db: Session, id: int, calendario_clinica: CalendarioClinicaUpdate
):
    db_calendario_clinica = (
        db.query(models.CalendarioClinica)
        .filter(models.CalendarioClinica.id == id)
        .first()
    )
