...
def validate_file(
    file_content: bytes, allowed_types: dict | None = None
) -> tuple[bool, str, str]:
    """
    ...
    if not mime_type or mime_type not in allowed_types:
        ...
        return (
            False,
            f"Tipo de arquivo não permitido. Tipos aceitos: {allowed_list}",
            "",
        )
    ...
    if mime_type in ALLOWED_IMAGE_TYPES and file_size > MAX_IMAGE_SIZE:
        return False, "Imagem muito grande. Tamanho máximo: 5MB", extension

    if mime_type in ALLOWED_DOCUMENT_TYPES and file_size > MAX_PDF_SIZE:
        return False, "PDF muito grande. Tamanho máximo: 10MB", extension
...
def upload_file(
    ...
    try:
        # Validar arquivo
        if file_type == "image":
            is_valid, error_msg, extension = validate_file(
                file_content, ALLOWED_IMAGE_TYPES
            )
        else:
            is_valid, error_msg, extension = validate_file(
                file_content, ALLOWED_DOCUMENT_TYPES
            )
        ...
        file_url = (
            f"{SUPABASE_STORAGE_URL}/storage/v1/object/public/{BUCKET_NAME}/{key}"
        )
        ...
        return False, f"Erro inesperado: {e!s}", None
