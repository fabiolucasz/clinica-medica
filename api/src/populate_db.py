    ...
    try:
        ...
        try:
            for nome, uf in estados:
                estado = (
                    db.query(models.Estados).filter(models.Estados.nome == nome).first()
                )
                if not estado:
...
def create_tipos_conselho():
    ...
    try:
        ...
        try:
            for tipo in tipos:
                conselho = (
                    db.query(models.Tipo_conselho)
                    .filter(models.Tipo_conselho.nome == tipo)
                    .first()
                )
                if not conselho:
    ...
    try:
        ...
        try:
            for esp in especialidades_data:
                especialidade = (
                    db.query(models.Especialidades)
                    .filter(models.Especialidades.nome == esp)
                    .first()
                )
    ...
    try:
        ...
        try:
            for turno in turnos_data:
                turno_obj = (
                    db.query(models.Turnos)
                    .filter(models.Turnos.nome == turno["nome"])
                    .first()
                )
                if not turno_obj:
...
def popular_calendario(ano_inicio=2026, ano_fim=2030):
    ...
    try:
        ...
        data_inicio = datetime(ano_inicio, 1, 1, tzinfo=agora_sp.tzinfo)
        data_fim = datetime(ano_fim, 12, 31, tzinfo=agora_sp.tzinfo)
        ...
        for clinica in clinicas:
            ...
            for dia in dias_ano:
                ...
                for feriado_nacional in feriados_nacionais:
                    if (
                        dia.mes == feriado_nacional["mes"]
                        and dia.dia == feriado_nacional["dia"]
                    ):
                        feriado = True
                ...
                if feriado:
                    ...
                    print(
                        f"    🎉 Feriado adicionado: {nome_feriado} em {dia.data_iso}"
                    )

        db.commit()
        print(
            f"✅ Calendário clínica populado para {len(clinicas)} clínicas - Ano {ano_atual}"
        )
...
def create_horario_consulta():
    ...
    try:
        ...
        try:
            ...
            if existing_count > 0:
                print(
                    f"  ℹ️  Já existem {existing_count} horários cadastrados. Pulando criação."
                )
                return True
            ...
            for id, hora_inicio, hora_fim, turno_id in horarios_data:
                ...
                existing = (
                    db.query(models.HorariosConsultas)
                    .filter(models.HorariosConsultas.id == id)
                    .first()
                )
                ...
                if not existing:
                    ...
                    db.add(horario)
                    print(
                        f"  ✅ Horário criado: {hora_inicio}-{hora_fim} (Turno {turno_id})"
                    )

            db.commit()
            print(
                f"  ✅ {len(horarios_data)} horários de consultas criados com sucesso!"
            )
            return True
